...
def convert_to_tfrecord(gt_mat, gt2_mat, gt4_mat, ms_mat, pan_mat, pan2_mat, pan4_mat, save_dir, name):
    
    filename = os.path.join(save_dir, name + '.tfrecords')
    
    gt = gt_mat['gt'][...]    ## ground truth N*H*W*C
    gt = np.array(gt,dtype = np.float32)
    gt2 = gt2_mat['gt2'][...]    ## ground truth N*H*W*C
    gt2 = np.array(gt2,dtype = np.float32)
    gt4 = gt4_mat['gt4'][...]    ## ground truth N*H*W*C
    gt4 = np.array(gt4,dtype = np.float32)
    pan = pan_mat['pan'][...]  #### Pan image N*H*W
    pan = np.array(pan, dtype = np.float32)
    pan2 = pan2_mat['pan2'][...]  #### Pan image N*H*W
    pan2 = np.array(pan2, dtype = np.float32)
    pan4 = pan4_mat['pan4'][...]  #### Pan image N*H*W
    pan4 = np.array(pan4, dtype = np.float32)
    ms = ms_mat['ms'][...] ### low resolution MS image
    ms = np.array(ms, dtype = np.float32) 
    n_samples = np.shape(pan)[0]
    
    if np.shape(gt)[0] != n_samples:
        raise ValueError('Ms size %d does not match pan size %d.' %(gt.shape[0], n_samples))
...
def read_and_decode_test(tfrecords_file, batch_size):
    ...
    img_features = tf.parse_single_example(
                                        ...
                                        features={
                                               ...
                                               'ms2_raw': tf.FixedLenFeature([], tf.string)
                                               })
    pan = tf.decode_raw(img_features['pan_raw'], tf.float32)
    pan = tf.reshape(pan, [512, 512, 3])
    pan2 = tf.decode_raw(img_features['pan2_raw'], tf.float32)
    pan2 = tf.reshape(pan2, [256, 256, 3])
    pan4 = tf.decode_raw(img_features['pan4_raw'], tf.float32)
    pan4 = tf.reshape(pan4, [128, 128, 3])
    gt = tf.decode_raw(img_features['gt_raw'], tf.float32)
    gt = tf.reshape(gt, [512,512,31])
    ms = tf.decode_raw(img_features['ms_raw'], tf.float32)
    ms = tf.reshape(ms, [64,64,31])
    ms2 = tf.decode_raw(img_features['ms2_raw'], tf.float32)
    ms2 = tf.reshape(ms2, [128, 128, 31])
    
    ##########################################################
    # you can put data augmentation here, I didn't use it
    ##########################################################
    # all the images of notMNIST are 28*28, you need to change the image size if you use other dataset.  
    pan_batch, pan2_batch, pan4_batch, gt_batch, ms_batch, ms2_batch = tf.train.batch([pan, pan2, pan4, gt, ms, ms2],
...
'''
gt_dir = './training_data/gt.mat'
gt2_dir = './training_data/gt2.mat'
gt4_dir = './training_data/gt4.mat'
ms_dir = './training_data/ms.mat'
pan_dir = './training_data/pan.mat'
pan2_dir = './training_data/pan2.mat'
pan4_dir = './training_data/pan4.mat'

save_dir = './training_data/'
BATCH_SIZE = 32

gt_mat = sio.loadmat(gt_dir)
gt2_mat = sio.loadmat(gt2_dir)
gt4_mat = sio.loadmat(gt4_dir)
ms_mat = sio.loadmat(ms_dir)
pan_mat = sio.loadmat(pan_dir)
pan2_mat = sio.loadmat(pan2_dir)
pan4_mat = sio.loadmat(pan4_dir)

name = 'trainlap'

#convert_to_tfrecord(gt_mat, gt2_mat, gt4_mat, ms_mat, pan_mat, pan2_mat, pan4_mat, save_dir, name)
...
tfrecords_file = './training_data/trainlap.tfrecords'
pan_batch, pan2_batch, pan4_batch, gt_batch,gt2_batch,gt4_batch, ms_batch = read_and_decode(tfrecords_file, batch_size=8)
...
with tf.Session(config=config)  as sess:
    ...
    try:
        while not coord.should_stop() and i<1:
            ...
            pan, gt, ms, gt2 = sess.run([pan_batch, gt_batch, ms_batch, gt2_batch])
            plot_images(pan)
            i+=1
